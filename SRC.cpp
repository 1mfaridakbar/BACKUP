...
struct line                               //STRUCTURE DEFINITION FOR DLL
{
	unsigned char string[101];          //TO STORE A LINE
...
void calcx()
{
	...
	{
		if(current->string[j]=='>' || current->string[j]==':'||current->string[j]==';'||current->string[j]=='f'||current->string[j]=='i'||current->string[j]=='l'||current->string[j]=='t')currx+=5;
		else if(current->string[j]=='!')currx+=2;
		else if(current->string[j]=='%')currx+=7;
		else if(current->string[j]=='.')currx+=4;
		else if(current->string[j]=='"'||current->string[j]=='('||current->string[j]==')'||current->string[j]=='['||current->string[j]==']')currx+=5;
		else currx+=6;
	}
}
...
setcolor(RED);
outtextxy(40,3,file);
outtextxy(3,ymax-15,"F1");              //        - DO -
outtextxy(66,ymax-15,"F2");             //        - DO -
outtextxy(146,ymax-15,"F3");            //        - DO -
outtextxy(203,ymax-15,"F4");            //        - DO -
}
...
{
setcolor(4);
outtextxy(xmax/2+100,3,"Preceeding line is too long");
}
if(curri==top-1)
{
top--;
...
void save_as()            // TO HANDLE SAVE_AS FUNCTION
{
char file_as[15];
getfile(file_as,0);
temp=head;
...
/*  THIS FUNCTION IS THE MAIN DRIVER OF TEH PROGRAM. IT USES VARIOUS
...
int j,tp,bm,lt,rt;
char typed;
fin.open(file,ios::in|ios::nocreate);       //OPEN THE FILE IN READ MODE
...
insert(typed);
break;
case 8 :save=0;       //BACK_SPACE PRESSED
bkspace();
break;
}
}while(1);
